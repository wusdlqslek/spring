		...
		<div class = "text-center">
			<ul class = "pagination">
				<c:if test="${pageMaker.prev}">
					<li><a href="listPage?board_page=${pagerMaker.startPage - 1}">prev</a></li>
				</c:if>
				
				<c:forEach begin ="${pageMaker.startPage}"
					end="${pageMaker.endPage}" var = "idx">
					<li <c:out value = "${pageMaker.cri.board_page ==idx? 'class=active':''}"/>>
					<a href = "listPage?board_page=${idx}">${idx}</a>
					</li>	
				</c:forEach>
				
				<c:if test = "${pageMaker.next && pageMaker.endPage>0}">
					<li><a href ="listPage?board_page=${pageMaker.endPage+1}">next</a></li>
				</c:if>
			</ul>
		</div>
				</div>
